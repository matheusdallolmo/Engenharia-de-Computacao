...
int checar_nulo(Spa_Mat* matriz, int linha, int coluna){
    int i, j, cont = 0;
    Cell* aux;
    ...
    for (i = 0; i < matriz->n_lin; i++){
        ...
        for (j; j < matriz->n_col; j++)
            cont++;
    }
    return ((linha * coluna) - cont);
}

int main() {
    int i, j, linha, coluna, elemento, qnt;

    scanf("%d %d", &linha, &coluna);

    Spa_Mat *matriz = criar(linha, coluna);

    for (i = 0; i < linha; i++){
            for(j = 0; j < coluna; j++){
                scanf("%d", &elemento);
                trocar(elemento, i, j, matriz);
            }
    }

    qnt =  checar_nulo(matriz, linha, coluna);
    printf("%d", qnt);
