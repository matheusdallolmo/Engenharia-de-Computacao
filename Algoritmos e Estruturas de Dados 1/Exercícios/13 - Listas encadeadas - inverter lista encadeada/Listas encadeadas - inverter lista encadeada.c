...
ListaE* inverter ( ListaE *normal){
    ListaE *invertida;
    Cell *aux;
    
    invertida = criar_listaE();
    
    if (!listaE_vazia(normal)){
        aux = normal->head;

        while (aux != NULL){
            inserir_primeiro(aux->item, invertida);

            aux = aux->next;
        }
    }
    return invertida;
    liberar_LE(invertida);
}


int main() {
    ListaE *normal, *invertida;
    int a;
    ...
    while(a!=-1){
        ...
        if(a!=-1)
            inserir_ultimo(a, normal);
    }
    
    invertida = inverter(normal);
    imprimir(invertida);
