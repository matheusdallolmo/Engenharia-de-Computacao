...
void imprimir_nao_folhas(NodeAVL *tree){
    ...
    imprimir_nao_folhas(tree->left);
    imprimir_nao_folhas(tree->right);
}

int main(){
    int i, N, x;
    int *grown = malloc(sizeof(int));
    NodeAVL *tree;
    ...
    for(i = 0; i < N; i++){
        ...
        if(i == 0)
            ...
            tree = inserirAVL(tree, x, grown);
    }
    
    imprimir_nao_folhas(tree);
