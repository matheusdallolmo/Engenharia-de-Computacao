...
int soma_folha(Node *tree){
    ...
    if((aux->left == NULL) && (aux->right == NULL)){
        ...
        return soma_folha(aux->right);    
    }else if(aux->right == NULL){
        return soma_folha(aux->left);
    }else{
        soma += soma_folha(aux->left);
        soma += soma_folha(aux->right);
        return soma;
    }
    return soma;
}
